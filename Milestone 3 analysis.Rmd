...
We will condunct an Analysis of Variance (ANOVA) since we have more than two means. We also need to adjust the $\alpha$ value with the Bonferroni correction. So the new p_value is calculated as $\alpha / K$, where $K = \dfrac{k(k-1)}{2}$. So, the new p-value is:
